...
let _ = 
  ...
  let _ = F.printf "RES: %a\n" Store.pp_v (interp p1) in (* true *)
  let _ = F.printf "AST: %a\n" pp p2 in
  let _ = F.printf "RES: %a\n" Store.pp_v (interp p2) in (* false *)
  let _ = F.printf "AST: %a\n" pp p3 in
  let _ = F.printf "RES: %a\n" Store.pp_v (interp p3) in (* true *)
  let _ = F.printf "AST: %a\n" pp p4 in
  let _ = F.printf "RES: %a\n" Store.pp_v (interp p4) in (* false *)
  let _ = F.printf "AST: %a\n" pp p5 in
  let _ = F.printf "RES: %a\n" Store.pp_v (interp p5) in (* false *)
  let _ = F.printf "AST: %a\n" pp p6 in
  let _ = F.printf "RES: %a\n" Store.pp_v (interp p6) in (* true *)
  let _ = F.printf "AST: %a\n" pp p7 in
