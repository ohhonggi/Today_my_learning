...
-- 팀별로 각 포지션별 선수의 수와 평균 신장(키)를 구하는

--create view Team_info as (
--    select *
--    from (select team_id, position, height from player)
--    pivot (count(*),
--        avg(height) as 평균신장 for position in 
--        ('GK' as GK,'DF' as DF,'MF' as MF,'FW' as FW))
--    ) order by 1;
--select * from Team_info;

--team_info에서 포지션이 미드인 사람의 선수의 수와 평균 신장 조회
--select team_id, position, countAll, datas
--from team_info
--unpivot include nulls 
--((countAll,datas) for position in ((MF ,MF_평균신장) as 'MF'))
--order by 1;

-- EMP 테이블에서 이메일 주소가 ‘소문자+숫자@소문자+숫자.소문자’ 패턴인
-- 직원의 직원번호, 직원이름, 직무, 이메일 주소를 출력하는 SQL문을 작성하라.
-- (단, REGEXP_LIKE 함수를 사용하며 POSIX 연산자와 PERL 정규 표현식 
-- 연산자 두 가지 방법을 모두 사용하여 출력

--SELECT empno, ename, job, email
--FROM emp
--where regexp_like(email,'[[:alnum:]]+@[[:alnum:]]+\.[a-z]+');
--
--select empno, ename, job, email
--from emp
--where REGEXP_LIKE(email, '\w+?@\w+?(\.\w)+?')

--select empno, ename, mobile, regexp_replace 
--    (mobile, '([[:digit:]]+)\.([[:digit:]]+)\.([[:digit:]]+)'
--    , '\1-\2-\3') as replaeN
--from emp;
--
--select empno, ename, mobile, regexp_replace 
--    (mobile, '(\d+?)\.(\d+?)\.(\d+?)', '\1-\2-\3') as replaeN
--from emp;
--
--select empno, ename, personal_homepage
--    ,regexp_replace(personal_homepage, 
--    'http://([[:alpha:]]+)\.([[:alpha:]]+)\.([[:alpha:]]+)/.+'
--    ,'http://\1.\2.\3/') as replaceE
--from emp
--where personal_homepage is not null;
--
--select empno, ename, personal_homepage
--    , regexp_replace(personal_homepage,
--    'http://(\w+?)\.(\w+?)\.(\w+?)/(.+)','http://\1.\2.\3/') as replaceE
--from emp
--where personal_homepage is not null;
